...
| `jerney-gke-nodes` | Node Pool | Spot e2-medium nodes, autoscales 1–3 |
...
- **1 node minimum** — autoscaler scales down to 1 when idle
...
```
1. terraform apply          ← provisions GKE cluster + VPC
         │
         ▼
2. gcloud get-credentials   ← configures local kubectl
         │
         ▼
3. helm install             ← installs platform tooling (ArgoCD, cert-manager,
   (post-setup.sh)             NGINX Ingress, Prometheus, Grafana, Loki)
         │
         ▼
4. kubectl apply            ← creates ArgoCD Application pointing to
   argocd-app-jerney.yaml      k8s/helm/jerney/ in GitHub
         │
         ▼
5. ArgoCD syncs             ← deploys frontend, backend, PostgreSQL
   helm chart                  from GitHub → cluster
         │
         ▼
6. CircleCI pipeline        ← on every git push to main:
   (on code push)              lint → sca → build → image-scan
                               → update-manifest (bumps image tag in values.yaml)
         │
         ▼
7. ArgoCD detects diff      ← polls GitHub every 3min, sees new image tag
   in values.yaml              in values.yaml, triggers rolling update
         │
         ▼
8. Prometheus scrapes       ← reads pod annotations:
   /metrics                    prometheus.io/scrape: "true"
                               prometheus.io/port: "5000"
         │
         ▼
9. Grafana shows            ← kube-prometheus-stack dashboards +
...
Run the same post-setup script used for EC2 (skip Section 1 — no kubeadm needed):
```bash
bash ../terraform-ec2/scripts/post-setup.sh
```

> The script installs: Metrics Server, NGINX Ingress, cert-manager, ArgoCD, SigNoz, Prometheus+Grafana, Loki.
...
| Nodes | Single EC2 t3.large | Spot e2-medium, autoscales 1–3 |
...
```
terraform-gke/
├── provider.tf       # google provider, required_version
